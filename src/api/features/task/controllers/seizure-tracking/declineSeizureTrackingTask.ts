...
export default function declineSeizureTrackingTask(req:Request, res:Response) {
    ...
    validateDeclineSeizureTrackingTaskRequest(requestData)
    .then((data)=> {
        ...
        .then((declinedTask)=> {
            ...
            .then((foundTask)=> {
                console.log("Seizure tracking task declined successfully");
                return sendSuccessResponse({ res, statusCode: 201, message:"Seizure tracking task declined successfully", data: { task: foundTask } })  
            })
            .catch((error)=> {
                console.log(error);
...
export function validateDeclineSeizureTrackingTaskRequest(data:IDeclineBowelMovementTaskRequest) {
    return new Promise<IDeclineBowelMovementTaskRequest>((resolve, reject)=> {
        ...
        if(!data.note) reject({ code:401, message:"Notes parameter must be provided" });
