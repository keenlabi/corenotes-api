...
export default function declineSkinIntegrityTask(req:Request, res:Response) {
    ...
    .then((data)=> {
        ...
        .then((declinedTask)=> {
            ...
            .then((foundTask)=> {
                console.log("Skin tntegrity task decline successfully");
                return sendSuccessResponse({ res, statusCode: 201, message:"Skin tntegrity task declined successfully", data: { task: foundTask } })  
            })
            .catch((error)=> {
                console.log(error);
