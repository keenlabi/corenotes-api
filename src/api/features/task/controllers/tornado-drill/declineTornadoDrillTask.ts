...
export default function declineTornadoDrillTask(req:Request, res:Response) {
    ...
    validateDeclineTornadoDrillTaskRequest(requestData)
    .then((data)=> {
        ...
        .then((declinedTask)=> {
            ...
            .then((foundTask)=> {
                console.log("Tornado drill task declined successfully");
                return sendSuccessResponse({ res, statusCode: 201, message:"Tornado drill task declined successfully", data: { task: foundTask } })  
            })
            .catch((error)=> {
                console.log(error);
...
export function validateDeclineTornadoDrillTaskRequest(data:IDeclineBowelMovementTaskRequest) {
    return new Promise<IDeclineBowelMovementTaskRequest>((resolve, reject)=> {
        ...
        if(!data.note) reject({ code: 422, message:"Notes parameter must be provided" });
