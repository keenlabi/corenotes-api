...
export default function saveIndividualAssessmentSession(req:Request, res:Response) {
    ...
    .then((updatedUser:IUser)=> {
        ...
        .then(async (foundAssessment)=> {
            ...
            sendSuccessResponse(res, 200, "Assessment session updated successfully", { individualAssessmentSession:  modifiedUpdatedAssessment })
        })
    })
    .catch((error)=> {
        console.log('INDIVIDUAL ASSESSMENT ERROR: There was an error updating the individual assessment session questions => ', error)
        sendFailureResponse(res, 500, "There was an error updating the individual assessment session questions");
    })
}