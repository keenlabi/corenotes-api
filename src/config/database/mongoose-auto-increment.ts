import { CallbackError, Connection, Schema } from "mongoose";
...
export function plugin (schema:Schema, options:any) {
    ...
    var settings = {
        model: '', // The model to configure the plugin for.
        field: '_id', // The field the plugin should track.
        startAt: 0, // The number the count should start at.
        incrementBy: 1, // The number by which to increment the count each time.
        unique: true // Should we create a unique index for the field
    },
        fields:any = {}, // A hash of fields to add properties to in Mongoose.
    ...
    var nextCount = function (callback:any) {
        IdentityCounter.findOne({
            model: settings.model,
            field: settings.field
        })
            .then((counter: { count: number; } | null) => {
    ...
    var resetCount = function (callback:any) {
            ...
            (function save() {
                ...
                if (ready) {
                    ...
                    if (typeof doc[settings.field] === 'number') {
                        ...
                        .then((updatedIdentityCounter: { count: number; }) => {
